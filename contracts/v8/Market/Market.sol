...
/// @title Market - A place where fellow gangstars come and get their debt.
// solhint-disable not-rely-on-time
contract Market is NonblockingLzApp {
  ...
  event GotNonBlockingLzReceive(uint16 _srcChainId, bytes _srcAddress, uint64 _nonce, bytes _payload);
  ...
  event LogUserDebtShare(uint256 LHS, uint256 RHS);
  event LogTotalDebtShare(uint256 LHS, uint256 RHS);
  event LogTotalDebtValue(uint256 LHS, uint256 RHS);
  ...
  function _borrow(address _to, uint256 _amount) internal checkDebtSize returns (uint256 _debtShare, uint256 _share) {
    ...
    _lzSend(lzDebtChainId, payload, payable(msg.sender), address(0x0), bytes(""));
  ...
  /// chain: debt
  /// @param _for Address of the user this payment should go.
  function repay(address _for, uint256 _debtValue) external payable nonReentrant accrue returns (uint256) {
    updateCollateralPrice();
    _repay(_for, _debtValue);
    return _debtValue;
  }
  ...
  function _nonblockingLzReceive(
    ...
    emit GotNonBlockingLzReceive(_srcChainId, _srcAddress, _nonce, _payload);
  ...
  function _isBorrowed(
    ...
    // 1. Find out debtShare from the give "_value" that msg.sender wish to borrow
    uint256 _debtShare = debtValueToShareLocal(_borrowAmount);

    // 2. Update user's debtShare
    userDebtShare[msg.sender] = userDebtShare[msg.sender] + _debtShare;

    emit LogUserDebtShare(userDebtShare[msg.sender], _userDebtShareForValidate);
    require(userDebtShare[msg.sender] == _userDebtShareForValidate, "userDebtShare on two chains are off-synced!");

    // 3. Book totalDebtShare and totalDebtValue
    totalDebtShare = totalDebtShare + _debtShare;
    emit LogTotalDebtShare(totalDebtShare, _totalDebtShareForValidate);
    ...
    emit LogTotalDebtValue(totalDebtValue, _totalDebtValueForValidate);
