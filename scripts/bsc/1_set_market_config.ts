...
async function main() {
  ...
  */
  ...
  console.log("(2) Verify market configs for ", PARAM.MARKET);
  ...
  const LZ_FUJI_CHAIN_ID = "10006";
  console.log("(4) set trust remote lzApp:", LZ_FUJI_CHAIN_ID, PARAM.DEBT_MARKET);
  tx = await market.setTrustedRemote(LZ_FUJI_CHAIN_ID, PARAM.DEBT_MARKET, { gasLimit: 7000000 });
  await tx.wait();
  console.log("tx hash:", tx.hash);
  console.log("✅ Done\n");
}
